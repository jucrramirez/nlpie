...
from .._similarity import downsample_square_matrix, reconstruct_similarity_matrix
from ..interpret.story import StoryData, build_story
from ..thresholds import SEVERITY_COLORS
...
_MAX_HEATMAP_SIZE = 300


def _add_caption(fig, caption: str, y_pos: float = -0.18) -> None:
    """Attach a small explanatory caption below a figure."""
    if not caption:
        return
    fig.add_annotation(
...
def _build_kpi_figure(go, report) -> object:
    ...
    if cards:
        ...
        for i, card in enumerate(cards):
            color = SEVERITY_COLORS[card.severity]
    ...
    fig.update_layout(
        title=dict(
            text=(
                ...
                f"<span style='font-size:11px;color:#555'>{model_name} — "
...
def _format_storytelling(story: StoryData) -> str:
    """Render shared storytelling data as Plotly annotation HTML."""
    if story.is_healthy:
        return (
            '<span style="color:#2ca02c;font-size:12px;font-weight:bold">'
            "✓ All metrics look healthy — no issues detected.</span>"
        )

    parts = [
        '<span style="font-size:13px;font-weight:bold;color:#333">Assessment Summary</span><br>'
        f'<span style="font-size:11px;color:#555">{story.headline}</span>'
    ]

    for group in story.groups:
        header = {"critical": "Critical Issues", "warning": "Warnings", "info": "Info"}[
            group.severity
        ]
        ...
        for exp in group.explanations:
            parts.append(
                ...
                f"<b>{exp.metric}</b>: {exp.summary}</span>"
            )
            if exp.detail:
                parts.append(
                    '<br><span style="font-size:10px;color:#666;'
                    f'padding-left:14px;display:inline-block">{exp.detail}</span>'
                )
            if exp.recommendation:
                parts.append(
                    '<br><span style="font-size:10px;color:#555;'
                    f'padding-left:14px;display:inline-block">→ {exp.recommendation}</span>'
                )

    if story.recommendations:
        parts.append(
            '<br><br><span style="font-size:12px;font-weight:bold;color:#333">'
            "Recommended Actions</span><br>"
        )
        for i, rec in enumerate(story.recommendations, 1):
            parts.append(f'<span style="font-size:10px;color:#444">{i}. {rec}</span><br>')
...
class PlotlyBackend(PlotBackend):
    ...
    def similarity_heatmap(
        ...
        if labels is not None:
            ...
            x_labels = [str(label) for label in sorted_labels]
            y_labels = [str(label) for label in sorted_labels]
        else:
    ...
    def hubness_bar_chart(self, counts: Sequence[int], top_n: int = 50) -> object:
        sorted_indices = sorted(range(len(counts)), key=lambda i: counts[i], reverse=True)[:top_n]
    ...
    def comparison_delta_heatmap(
        ...
        fig = self._go.Figure(
            data=self._go.Heatmap(
                ...
                zmid=0,
                text=[
                    [f"{v:+.1%}" if v is not None else "n/a" for v in row] for row in delta_matrix
                ],
                texttemplate="%{text}",
            )
        )
    ...
    def _build_chart_list(self, report) -> list[tuple[str, object]]:
        ...
        if getattr(report, "intrinsic", None) is not None:
            ...
            if sims:
                fig = self.similarity_distribution(
                    sims, report.intrinsic.mean, report.intrinsic.std
                )
        ...
        if getattr(report, "geometry", None) is not None:
            ...
            if counts:
                fig = self.hubness_histogram(
                    counts, report.geometry.hubness_skewness, report.geometry.hubness_k
                )
        ...
        if pairwise and n > 1:
            # Raises ValueError for malformed reports whose triangle length
            # does not match n * (n - 1) / 2.
            matrix = reconstruct_similarity_matrix(pairwise, n)
            matrix = downsample_square_matrix(matrix, _MAX_HEATMAP_SIZE)
            fig = cast(Any, self.similarity_heatmap(matrix))
    ...
    def _build_storytelling_figure(self, explanations, report) -> object:
        ...
        text = _format_storytelling(build_story(explanations))
    ...
    def full_dashboard(self, report, interpretation=None) -> Dashboard:
        fig_kpi = _build_kpi_figure(self._go, report)
        ...
        return Dashboard(kpi=fig_kpi, charts=chart_list, story=fig_storytelling)
